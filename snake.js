...
class SnakeGame {
  ...
  update() {
    ...
    this.loopCheck.push(`${this.snake[0].x},${this.snake[0].y}`);
    if (this.loopCheck.length > 50) this.loopCheck.shift();

    if (this.strategy === "smart" &&
        this.growthCounter > 100 && new Set(this.loopCheck).size < 10) {
      return this.restart();
    }
